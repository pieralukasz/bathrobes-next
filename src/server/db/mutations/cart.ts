...
export async function addItemToCart(basketId: number, productSizeId: number, quantity: number) {
  return await db.insert(basketItems).values({ basketId, productSizeId, quantity }).returning();
}

// Update the quantity of an item in the cart
export async function updateCartItem(basketItemId: number, quantity: number) {
  return await db.update(basketItems).set({ quantity }).where(eq(basketItems.id, basketItemId)).returning();
}
